...
void mostraTabela () {
    ...
    {
        printf("\n%d | %30s | %3d | %s | %3c | %3c | %3d | %3d | ", i, tabSimb[i].id, tabSimb[i].end, tabSimb[i].tip == INT ? "INT" : "LOG", tabSimb[i].cat, tabSimb[i].esc, tabSimb[i].rot, tabSimb[i].npa);
...
int encontra_func(){
    int i;
    for(i = posTab; tabSimb[i].cat != 'f'; i--);
    return i;
}